...
public class AdapterPetReptile extends RecyclerView.Adapter<AdapterPetReptile.AdapterPetsViewHolder> {
    ...
    public AdapterPetReptile(Context context, OnItemClickListener listener, DataPets dataPets) {
    ...
    public void onBindViewHolder(@NonNull AdapterPetsViewHolder holder, int position) {
        DataItem dataItem = dataPets.dataReptilePet(context).get(position);
        holder.bind(dataItem, listener);
    }

    @Override
    public int getItemCount() {
        return dataPets.dataReptilePet(context).size();
    }
