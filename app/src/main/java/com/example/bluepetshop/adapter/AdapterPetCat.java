...
public class AdapterPetCat extends RecyclerView.Adapter<AdapterPetCat.AdapterPetsViewHolder> {
    ...
    public AdapterPetCat(Context context, OnItemClickListener listener, DataPets dataPets) {
    ...
    public void onBindViewHolder(@NonNull AdapterPetsViewHolder holder, int position) {
        DataItem dataItem = dataPets.dataCatPet(context).get(position);
        holder.bind(dataItem, listener);
    }

    @Override
    public int getItemCount() {
        return dataPets.dataCatPet(context).size();
    }
