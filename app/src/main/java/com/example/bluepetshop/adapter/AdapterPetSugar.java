...
public class AdapterPetSugar extends RecyclerView.Adapter<AdapterPetSugar.AdapterPetsViewHolder> {
    ...
    public AdapterPetSugar(Context context, OnItemClickListener listener, DataPets dataPets) {
    ...
    public void onBindViewHolder(@NonNull AdapterPetsViewHolder holder, int position) {
        DataItem dataItem = dataPets.dataSugarPet(context).get(position);
        holder.bind(dataItem, listener);
    }

    @Override
    public int getItemCount() {
        return dataPets.dataSugarPet(context).size();
    }
